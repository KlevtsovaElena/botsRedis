...
func getUsersOfBDAndWriteInRedis() {
	...
	// подключимся к редис
	// и записшем юзеров, а также их количество

	if len(UsersDB) < 1 {
		RedisDB.Set("countUsers", 0, 0)
		val2, _ := RedisDB.Get("countUsers").Result()
		fmt.Println("количество юзеров", string(val2))
		return
	}

	messagesForRedis, err := json.Marshal(UsersDB)
	if err != nil {
		fmt.Print("не смогли закодировать в json мапу")
	}

	countMessages := len(UsersDB)
	RedisDB.Set("users", messagesForRedis, 0)
	RedisDB.Set("countUsers", countMessages, 0)
	val, _ := RedisDB.Get("users").Result()
	val2, _ := RedisDB.Get("countUsers").Result()
	fmt.Println("юзеры", string(val))
	fmt.Println("количество юзеров", string(val2))
}

func getCountMessagesOfBDAndWriteInRedis() {
	//получили количество сообщений из базы в оперативную память
	var countMessages int
	countRow := Db.QueryRow("select count(id) from `messages`")
	err := countRow.Scan(&countMessages)
	if err != nil {
		fmt.Println("ошибка сканирования сообщений", err)
	}

	if countMessages < 1 {
		RedisDB.Set("countMessages", 0, 0)
		val2, _ := RedisDB.Get("countMessages").Result()
		fmt.Println("количество сообщений", string(val2))
		return
	}
	RedisDB.Set("countMessages", countMessages, 0)
...
func main() {
	...
	//получили юзеров из базы в оперативную память
	getUsersOfBDAndWriteInRedis()

	//получили количество сообщений из базы в оперативную память
	getCountMessagesOfBDAndWriteInRedis()
...
func handleBot(j int) {
	...
	for i := 0; i < number; i++ {
		...
		if exist == false {
			...
			// записываем в оперативку и редис
			UsersDB[chatId] = user
		...
		if err != nil {
			...
			// обновим количество сообщений в редис
			count, errRedis := RedisDB.Get("countMessages").Result()
			if errRedis == redis.Nil {
				fmt.Println("Key not found")
				RedisDB.Set("countMessages", "1", 0)
				...
				countInt += 1
				RedisDB.Set("countMessages", countInt, 0)
				fmt.Println("есть новое сообщение", "всего сообщений из редис", countInt)
			}
